...
async function healthState(home, now) {
  const filePath = path2.join(home, ".claude", "status-cache", "anthropic");
  const attemptPath = path2.join(home, ".claude", "status-cache", "anthropic-attempt");
  await repairPrivateDirectory(path2.dirname(filePath));
  let indicator = "";
  let mtime = 0;
  try {
    await ensurePrivateFile(filePath);
    indicator = (await fs2.readFile(filePath, "utf8")).trim();
    mtime = Math.floor((await fs2.stat(filePath)).mtimeMs / 1e3);
  } catch {
  }
  try {
    await ensurePrivateFile(attemptPath);
    mtime = Math.max(mtime, Math.floor((await fs2.stat(attemptPath)).mtimeMs / 1e3));
  } catch {
  }
  return {
    indicator,
...
async function acquireHealthRefreshLease(home, now = Date.now()) {
  const directory = path2.join(home, ".claude", "status-cache");
  const lockPath = path2.join(directory, "anthropic-refresh.lock");
  await ensurePrivateDirectory(directory);
  for (let attempt = 0; attempt < 2; attempt += 1) {
...
`);
      ...
      if (error.code !== "EEXIST")
        return null;
      try {
        const before = await fs2.stat(lockPath);
        if (now - before.mtimeMs < HEALTH_REFRESH_LOCK_STALE_MS)
          return null;
        const after = await fs2.stat(lockPath);
...
async function releaseHealthRefreshLease(lease) {
  try {
    const before = await fs2.stat(lease.path);
    if ((await fs2.readFile(lease.path, "utf8")).trim() !== lease.token)
      return;
    const after = await fs2.stat(lease.path);
    if (before.dev !== after.dev || before.ino !== after.ino)
      return;
    await fs2.unlink(lease.path);
  } catch {
  }
}
async function spawnHealthRefresh(cliPath, home = os2.homedir()) {
  let lease;
  try {
...
async function refreshAnthropicHealth(home = os2.homedir(), lease) {
  const directory = path2.join(home, ".claude", "status-cache");
  const filePath = path2.join(directory, "anthropic");
  const attemptPath = path2.join(directory, "anthropic-attempt");
  try {
    await ensurePrivateDirectory(directory);
    ...
    if (!response.ok)
      throw new Error(`Anthropic status: HTTP ${response.status}`);
    const body = await response.json();
    const indicator = body.status?.indicator;
    if (!indicator)
      throw new Error("Anthropic status: missing indicator");
...
function transcriptCachePath(home, transcriptPath) {
  ...
  return path3.join(home, ".agent-hud", "transcript-cache", `${key}.json`);
}
async function readTranscriptCache(filePath, transcriptPath) {
  try {
    await ensurePrivateFile(filePath);
    const cached = JSON.parse(await fs3.readFile(filePath, "utf8"));
...
async function readTranscriptFacts(transcriptPath, home) {
  if (!transcriptPath)
    return null;
  try {
    const stat = await fs3.stat(transcriptPath);
    const cachePath = transcriptCachePath(home, transcriptPath);
    const cacheDirectory = path3.dirname(cachePath);
    ...
    const after = await fs3.stat(transcriptPath);
    const result = {
      turns: ids.size,
      cacheTtlSeconds,
...
async function effortFromSettings(cwd, home) {
  const candidates = [
    path4.join(cwd, ".claude", "settings.local.json"),
    path4.join(cwd, ".claude", "settings.json"),
    path4.join(home, ".claude", "settings.local.json"),
    path4.join(home, ".claude", "settings.json")
  ];
  for (const candidate of candidates) {
    try {
      const settings = JSON.parse(await fs4.readFile(candidate, "utf8"));
...
async function appendMetric(filePath, row) {
  try {
    await ensurePrivateDirectory(path4.dirname(filePath));
    await fs4.appendFile(filePath, row, { encoding: "utf8", mode: 384 });
    await ensurePrivateFile(filePath);
  } catch {
  }
}
async function readMetricRows(filePath) {
  try {
    await repairPrivateDirectory(path4.dirname(filePath));
    await ensurePrivateFile(filePath);
    return (await fs4.readFile(filePath, "utf8")).trim().split("\n").filter(Boolean).map((line) => line.split("	"));
  } catch {
    return [];
  }
}
...
async function compactTelemetry(input, turns, home, now, writeLogs, options) {
  ...
  const filePath = path4.join(home, ".claude", "context-log", `${metricSessionName(sessionId)}.tsv`);
...
async function logCost(input, home, now) {
  ...
  const filePath = path4.join(home, ".claude", "cost-log", `${metricSessionName(sessionId)}.tsv`);
...
function toolTarget(name, toolInput) {
  ...
  if (typeof rawPath === "string")
    return safeText(path5.basename(rawPath), 48);
...
async function candidateFiles(dataDir) {
  let entries;
  try {
    entries = await fs5.readdir(dataDir, { withFileTypes: true });
  } catch {
    return [];
  }
  const files = [];
  for (const entry of entries) {
    if (!entry.isFile() || !entry.name.endsWith(".jsonl"))
      continue;
    const filePath = path6.join(dataDir, entry.name);
    try {
...
async function canonicalPath(value) {
  try {
    return await fs5.realpath(value);
  } catch {
...
function snapshotFromClaude(facts, git = null) {
  ...
  return {
    ...
    model: facts.model,
    cwd,
    project: cwd ? path7.basename(cwd) : "",
...
function displayPath(cwd) {
  ...
  if (cwd === home) return "~";
  if (cwd.startsWith(`${home}${path8.sep}`)) return `~/${cwd.slice(home.length + 1)}`;
  return cwd;
}
...
var ROOT = path9.resolve(path9.dirname(fileURLToPath(import.meta.url)), "..");
var CLI_PATH = path9.join(ROOT, "dist", "cli.js");
var CODEX_PRESETS = {
  compact: {
    status_line: [
...
async function writeWithBackup(filePath, content, dryRun = false) {
  let original = "";
  try {
    original = await fs6.readFile(filePath, "utf8");
  } catch (error) {
  ...
  if (original) {
    ...
    await fs6.copyFile(filePath, backup);
  }
  const temporary = `${filePath}.${process.pid}.tmp`;
  await fs6.writeFile(temporary, content, { encoding: "utf8", mode: 384 });
  await fs6.rename(temporary, filePath);
  return { changed: true, backup, content };
}
async function setupCodex(options = {}) {
  const filePath = options.config || path9.join(process.env.CODEX_HOME || path9.join(os5.homedir(), ".codex"), "config.toml");
  let original = "";
  try {
    original = await fs6.readFile(filePath, "utf8");
  } catch (error) {
...
async function setupClaude(options = {}) {
  const root = process.env.CLAUDE_CONFIG_DIR || path9.join(os5.homedir(), ".claude");
  const filePath = options.config || path9.join(root, "settings.json");
  let original = "";
  try {
    original = await fs6.readFile(filePath, "utf8");
  } catch (error) {
...
async function setupCursor(options = {}) {
  const root = process.env.CURSOR_CONFIG_DIR || path9.join(os5.homedir(), ".cursor");
  const filePath = options.config || path9.join(root, "cli-config.json");
  const hooksFilePath = options.hooksConfig || path9.join(root, "hooks.json");
...
async function setupAntigravity(options = {}) {
  const settingsRoot = process.env.ANTIGRAVITY_CONFIG_DIR || path9.join(os5.homedir(), ".gemini", "antigravity-cli");
  const hooksRoot = process.env.GEMINI_CUSTOMIZATION_DIR || path9.join(os5.homedir(), ".gemini", "config");
  const filePath = options.config || path9.join(settingsRoot, "settings.json");
  const hooksFilePath = options.hooksConfig || path9.join(hooksRoot, "hooks.json");
...
`);
  ...
  if (facts.healthCacheStale) await spawnHealthRefresh(CLI_PATH2);
...
async function refreshHealth(args) {
...
function isMainModule() {
  ...
  try {
    return fs7.realpathSync(fileURLToPath2(import.meta.url)) === fs7.realpathSync(executable);
  } catch {
    return path10.resolve(fileURLToPath2(import.meta.url)) === path10.resolve(executable);
  }
}
if (isMainModule()) {
  main().catch((error) => {
